...
class NeRFVolumeRenderer(VolumeRenderer):
    ...
    def forward(
        ...
        true_global_step: Optional[int] = None,
        **kwargs
        ...
        if self.training:
            geo_out = self.geometry(
                positions, output_normal=self.material.requires_normal, output_cmm=True, get_new_bbox=(true_global_step % 200 == 0)
            )
        ...
        if self.training:
            ...
            if "normal" in geo_out:
                if self.cfg.return_comp_normal:
                    ...
                    gb_normal_viewspace = torch.einsum("bij,bhwj->bhwi", w2c, comp_normal.reshape(batch_size, height, width, 3)).reshape(-1, 3)
                    gb_normal_viewspace = gb_normal_viewspace * opacity
                    gb_normal_viewspace[(gb_normal_viewspace.sum(dim=-1) == 0), -1] = 1
                    ...
                    gb_normal_viewspace = (gb_normal_viewspace + 1.0 ) / 2.0
                    gb_normal_viewspace[..., 0] = (
                        1 - gb_normal_viewspace[..., 0]
                    )  # Flip the sign on the x-axis to match bae system
                    out.update(
                        {
                            "comp_normal_viewspace": gb_normal_viewspace.view(batch_size, height, width, 3)
                        }
                    )
            ...
            if "cmm" in geo_out:
                comp_cmm = nerfacc.accumulate_along_rays(
                    weights[..., 0],
                    values=geo_out["cmm"],
                    ray_indices=ray_indices,
                    n_rays=n_rays,
                )
                comp_cmm = torch.cat([comp_cmm, opacity * 2 - 1], dim=-1)
                out.update(
                    {
                        "comp_cmm": comp_cmm.view(batch_size, height, width, 4),
                    }
                )
        else:
            if "normal" in geo_out:
            ...
            if "cmm" in geo_out:
                ...
                # comp_cmm = comp_cmm * opacity  # for visualization
                comp_cmm = (comp_cmm + 1.0) / 2.0 * opacity  # for visualization
                out.update(
                    {
                        "comp_cmm": comp_cmm.view(batch_size, height, width, 3),
                    }
                )
