...
class ImplicitVolume(BaseImplicitGeometry):
    ...
    def forward(
        self, points: Float[Tensor, "*N Di"], output_normal: bool = False, output_cmm: bool = False, get_new_bbox: bool = False,
        ...
        if output_cmm:
            with torch.no_grad():
                try:
                    if get_new_bbox:
                        mesh_coarse = self.isosurface()
                        vmin, vmax = mesh_coarse.v_pos.amin(dim=0), mesh_coarse.v_pos.amax(dim=0)
                        self.vmin, self.vmax = vmin * 1.1, vmax * 1.1
                    else:
                        vmin, vmax = self.vmin, self.vmax
                except Exception as e:
                    threestudio.warn(f"Failed to get bounding box: {e}")
                    mesh_coarse = self.isosurface()
                    vmin, vmax = mesh_coarse.v_pos.amin(dim=0), mesh_coarse.v_pos.amax(dim=0)
                    self.vmin, self.vmax = vmin * 1.1, vmax * 1.1
            ...
            center = (vmin + vmax) / 2
            scale = (vmax - vmin) / 2
            cmm = (points_unscaled - center) / scale
            cmm = cmm[..., [1, 0, 2]]
            cmm[..., 1] = - cmm[..., 1]
            output.update({"cmm": cmm})
